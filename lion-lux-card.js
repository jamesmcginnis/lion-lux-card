/**
 * Lion Lux Card
 * Compact pill card showing current lux / illuminance levels at a glance.
 * Tap pill → sensor overview popup with per-sensor lux value and 24-hour history.
 * GitHub: https://github.com/jamesmcginnis/lion-lux-card
 */

// ─── Editor: Colour field definitions ────────────────────────────────────────
const LION_COLOUR_FIELDS = [
  ...
  { key: 'fill_color',   label: 'Pill Fill',        desc: 'Colour of the fill bar that grows with brightness.',               default: '#FFD60A' },
  { key: 'high_color',   label: 'High',             desc: 'Colour used to indicate a sensor is above the high threshold.',    default: '#FF9F0A' },
  { key: 'low_color',    label: 'Low',              desc: 'Colour used to indicate a sensor is below the low threshold.',     default: '#48484A' },
  { key: 'popup_bg',     label: 'Popup Background', desc: 'Background colour of all popup dialogs.',                          default: '#1c1c1e' },
  { key: 'icon_color',   label: 'Sensor Icon',      desc: 'Colour of the lux icon on the pill card.',                        default: '#FFD60A' },
];

// ─── SVG paths ────────────────────────────────────────────────────────────────
// Brightness / sun icon
const LION_SUN_PATH = 'M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0 .39-.39.39-1.03 0-1.41l-1.06-1.06zm1.06-10.96c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z';
// Dimmed / low-light icon
const LION_SUN_DIM_PATH = 'M12 9c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3zm0-4.5V3c0-.55-.45-1-1-1s-1 .45-1 1v1.5c0 .55.45 1 1 1s1-.45 1-1zm0 15V18c0-.55-.45-1-1-1s-1 .45-1 1v1.5c0 .55.45 1 1 1s1-.45 1-1zM5.99 4.58c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0 .39-.39.39-1.03 0-1.41l-1.06-1.06zm1.06-10.96c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM3 12c0 .55.45 1 1 1h1.5c.55 0 1-.45 1-1s-.45-1-1-1H4c-.55 0-1 .45-1 1zm15.5 0c0 .55.45 1 1 1H21c.55 0 1-.45 1-1s-.45-1-1-1h-1.5c-.55 0-1 .45-1 1z';

// ─────────────────────────────────────────────────────────────────────────────
//  Main Card
// ─────────────────────────────────────────────────────────────────────────────
class LionLuxCard extends HTMLElement {
  constructor() {
    super();
  ...
  static getConfigElement() {
    return document.createElement('lion-lux-card-editor');
  }
  ...
  _isHigh(entityId) {
    ...
    return lux >= (this._config.high_threshold ?? 1000);
  }

  _luxLabel(lux) {
    if (lux === null) return 'Unknown';
    if (lux >= 10000) return `${(lux / 1000).toFixed(1)}k lx`;
    if (lux >= 1000)  return `${(lux / 1000).toFixed(2).replace(/\.?0+$/, '')}k lx`;
  ...
  _unit(entityId) {
    return this._hass?.states[entityId]?.attributes?.unit_of_measurement || 'lx';
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        #pill-fill {
          ...
          background: rgba(${this._hexToRgb(cfg.fill_color || '#FFD60A')}, 0.22);
  ...
  _update() {
    ...
    const highThreshold = cfg.high_threshold ?? 1000;
    const fillMax       = cfg.fill_max       ?? 10000;
    const luxValues     = entities.map(e => this._lux(e)).filter(v => v !== null);
    const highCount     = entities.filter(e => this._isHigh(e)).length;
    const total         = entities.length;
    const label         = (cfg.title || '').trim();

    // Average lux for pill summary
    const avgLux = luxValues.length
      ? luxValues.reduce((a, b) => a + b, 0) / luxValues.length
      : null;

    const countTxt = avgLux === null
      ? 'No readings'
      : highCount === 0
        ? `Avg ${this._luxLabel(avgLux)} · All Low`
        : highCount === total
          ? `Avg ${this._luxLabel(avgLux)} · All Bright`
          : `Avg ${this._luxLabel(avgLux)} · ${highCount} Bright`;

    content.innerHTML = label
      ? `<span class="label">${label}</span><span class="count">${countTxt}</span>`
      : `<span class="count">${countTxt}</span>`;

    // Fill bar based on max lux reading across all sensors, capped at fill_max
    const maxLux  = luxValues.length ? Math.max(...luxValues) : 0;
    const fillPct = Math.min(100, Math.round((maxLux / fillMax) * 100));
    const fillEl  = this.shadowRoot.getElementById('pill-fill');
    if (fillEl) {
      fillEl.style.width        = `${fillPct}%`;
      fillEl.style.borderRadius = fillPct >= 100 ? '28px' : '28px 0 0 28px';
    }
  }
  ...
  _openOverviewPopup() {
    ...
    const lowCol   = cfg.low_color     || '#48484A';
    ...
    style.textContent = `
      @keyframes lionFadeIn  { from{opacity:0} to{opacity:1} }
      @keyframes lionSlideUp { from{transform:translateY(24px) scale(0.97);opacity:0} to{transform:none;opacity:1} }
      .lion-popup   { animation: lionSlideUp 0.28s cubic-bezier(0.34,1.28,0.64,1); }
      #lion-overlay { animation: lionFadeIn 0.2s ease; }
      .lion-sensor-pill {
      ...
      .lion-sensor-pill.is-high { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.3); }
      .lion-sensor-pill.pressing { transform: scale(0.93); }
      .lion-pill-name { font-size: 10px; font-weight: 600; color: rgba(255,255,255,0.45); text-align: center; letter-spacing: 0.02em; line-height: 1.3; max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .lion-close-btn:hover { background:rgba(255,255,255,0.22)!important; }
      .lion-stat-pill { flex:1; cursor:pointer; transition:background 0.15s,border-color 0.15s; }
    `;

    const popup = document.createElement('div');
    popup.className = 'lion-popup';
    ...
    headerRow.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;">
        <div style="width:28px;height:28px;border-radius:50%;background:${accent}22;display:flex;align-items:center;justify-content:center;">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="${accent}"><path d="${LION_SUN_PATH}"/></svg>
        </div>
        <span style="font-size:15px;font-weight:700;color:${textCol};">${cfg.title || 'Lux Sensors'}</span>
      </div>
      <button class="lion-close-btn" style="background:rgba(255,255,255,0.1);border:none;border-radius:50%;width:30px;height:30px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,0.65);font-size:16px;line-height:1;padding:0;transition:background 0.15s;flex-shrink:0;">✕</button>`;
    headerRow.querySelector('.lion-close-btn').addEventListener('click', () => this._closeOverviewPopup());

    // ── Pill map ──────────────────────────────────────────────────────────
    const pillMap = new Map(); // entityId → { pill, luxEl, stateEl }

    // ── Stats row ─────────────────────────────────────────────────────────
    const highStatValueEl   = document.createElement('div');
    const avgStatValueEl    = document.createElement('div');
    const peakStatValueEl   = document.createElement('div');
    highStatValueEl.style.cssText  = `font-size:17px;font-weight:700;letter-spacing:-0.3px;color:${textCol};`;
    avgStatValueEl.style.cssText   = `font-size:17px;font-weight:700;letter-spacing:-0.3px;color:${textCol};`;
    peakStatValueEl.style.cssText  = `font-size:17px;font-weight:700;letter-spacing:-0.3px;color:${textCol};`;
    ...
    const highlightPills = (mode) => {
      pillMap.forEach(({ pill }, entityId) => {
        const high  = this._isHigh(entityId);
        const match = mode === null || (mode === 'high' && high) || (mode === 'low' && !high);
    ...
    const makeStatPill = (label, valueEl, mode) => {
      ...
      el.style.cssText = `flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:10px 8px;text-align:center;cursor:${mode ? 'pointer' : 'default'};transition:background 0.15s,border-color 0.15s;`;
      ...
      if (mode) {
        el.classList.add('lion-stat-pill');
        ...
        el.addEventListener('click', ev => {
          ...
          statsRow.querySelectorAll('.lion-stat-pill').forEach(p => {
    ...
    const refreshOverview = () => {
      const luxValues = entities.map(e => this._lux(e)).filter(v => v !== null);
      const highCount = entities.filter(e => this._isHigh(e)).length;
      const avgLux    = luxValues.length ? luxValues.reduce((a, b) => a + b, 0) / luxValues.length : null;
      const peakLux   = luxValues.length ? Math.max(...luxValues) : null;

      highStatValueEl.textContent  = `${highCount} / ${entities.length}`;
      avgStatValueEl.textContent   = avgLux  !== null ? this._luxLabel(avgLux)  : '—';
      peakStatValueEl.textContent  = peakLux !== null ? this._luxLabel(peakLux) : '—';

      pillMap.forEach(({ pill, luxEl, stateEl }, entityId) => {
        const high = this._isHigh(entityId);
        const lux  = this._lux(entityId);
        pill.classList.toggle('is-high', high);
        pill.style.background  = high ? 'rgba(255,255,255,0.1)' : '';
        pill.style.borderColor = high ? 'rgba(255,255,255,0.3)' : '';
        luxEl.style.color  = high ? highCol : 'rgba(255,255,255,0.25)';
        luxEl.textContent  = lux !== null ? this._luxLabel(lux) : '—';
        stateEl.style.color = high ? highCol : 'rgba(255,255,255,0.25)';
        stateEl.textContent = high ? 'Bright' : 'Low';
      });
    };
    this._refreshOverview = refreshOverview;
    refreshOverview();

    statsRow.appendChild(makeStatPill('Bright',  highStatValueEl,  'high'));
    statsRow.appendChild(makeStatPill('Average', avgStatValueEl,   null));
    statsRow.appendChild(makeStatPill('Peak',    peakStatValueEl,  null));
    ...
    const buildPill = (entityId) => {
      ...
      pill.className = `lion-sensor-pill${high ? ' is-high' : ''}`;
      if (high) { pill.style.background = 'rgba(255,255,255,0.1)'; pill.style.borderColor = 'rgba(255,255,255,0.3)'; }

      const svgEl = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      svgEl.setAttribute('width', '22'); svgEl.setAttribute('height', '22');
      svgEl.setAttribute('viewBox', '0 0 24 24');
      svgEl.setAttribute('fill', high ? highCol : 'rgba(255,255,255,0.2)');
      svgEl.style.cssText = 'display:block;flex-shrink:0;';
      const pathEl = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      pathEl.setAttribute('d', high ? LION_SUN_PATH : LION_SUN_DIM_PATH);
      svgEl.appendChild(pathEl);

      const luxEl = document.createElement('div');
      luxEl.style.cssText = `font-size:12px;font-weight:700;color:${high ? highCol : 'rgba(255,255,255,0.25)'};line-height:1;`;
      luxEl.textContent = lux !== null ? this._luxLabel(lux) : '—';

      const stateEl = document.createElement('div');
      stateEl.style.cssText = `font-size:10px;font-weight:600;color:${high ? highCol : 'rgba(255,255,255,0.2)'};line-height:1;`;
      stateEl.textContent = high ? 'Bright' : 'Low';

      const nameEl = document.createElement('div');
      nameEl.className = 'lion-pill-name';
      nameEl.textContent = name;

      pill.appendChild(svgEl);
      pill.appendChild(luxEl);
      pill.appendChild(stateEl);
      pill.appendChild(nameEl);
      pill.dataset.entityId = entityId;

      pillMap.set(entityId, { pill, luxEl, stateEl });
  ...
  _openSensorPopup(entityId) {
    ...
    const style = document.createElement('style');
    style.textContent = `
      @keyframes lionSensorUp { from{transform:translateY(30px) scale(0.96);opacity:0} to{transform:none;opacity:1} }
      .lion-sensor-popup { animation: lionSensorUp 0.32s cubic-bezier(0.34,1.2,0.64,1); user-select:none; -webkit-user-select:none; }
      .lion-hk-row { display:flex; align-items:center; justify-content:space-between; padding:12px 16px; cursor:default; }
      .lion-hk-row.tappable { cursor:pointer; }
      .lion-hk-row.tappable:active { background:rgba(255,255,255,0.05); }
      .lion-hk-row-label { font-size:15px; font-weight:500; color:rgba(255,255,255,0.85); }
      .lion-hk-row-value { font-size:15px; font-weight:500; color:rgba(255,255,255,0.4); display:flex; align-items:center; gap:6px; }
      .lion-hk-chevron { font-size:13px; color:rgba(255,255,255,0.25); }
    `;

    const popup = document.createElement('div');
    popup.className = 'lion-sensor-popup';
    ...
    const getIsHigh       = () => this._isHigh(entityId);
    const getLux          = () => this._lux(entityId);
    ...
    const applyStateCircle = (isHigh, luxVal) => {
      const col = isHigh ? highCol : lowCol;
      stateCircle.style.cssText = `width:80px;height:80px;border-radius:50%;background:${isHigh ? `${col}22` : 'rgba(255,255,255,0.07)'};border:2.5px solid ${col};display:flex;align-items:center;justify-content:center;transition:background 0.25s,border-color 0.25s;box-shadow:${isHigh ? `0 4px 24px ${col}55` : 'none'};`;
      stateCircle.innerHTML = `<svg width="38" height="38" viewBox="0 0 24 24" fill="${col}" style="opacity:${isHigh ? '1' : '0.4'}"><path d="${isHigh ? LION_SUN_PATH : LION_SUN_DIM_PATH}"/></svg>`;
    };
    applyStateCircle(getIsHigh(), getLux());

    const luxLabel = document.createElement('div');
    const applyLuxLabel = (isHigh, luxVal) => {
      luxLabel.style.cssText = `font-size:26px;font-weight:700;color:${isHigh ? highCol : 'rgba(255,255,255,0.6)'};letter-spacing:-1px;`;
      luxLabel.textContent = luxVal !== null ? this._luxLabel(luxVal) : '—';
    };
    applyLuxLabel(getIsHigh(), getLux());

    const stateLabel = document.createElement('div');
    const applyStateLabel = (isHigh) => {
      stateLabel.style.cssText = `font-size:14px;font-weight:600;color:${isHigh ? highCol : 'rgba(255,255,255,0.35)'};letter-spacing:0.01em;`;
      stateLabel.textContent = isHigh ? 'Bright' : 'Low';
    };
    applyStateLabel(getIsHigh());

    stateWrap.appendChild(stateCircle);
    stateWrap.appendChild(luxLabel);
    ...
    // Device class / unit row
    const dc   = this._deviceClass(entityId);
    const unit = this._unit(entityId);
    const typeRow = document.createElement('div');
    typeRow.className = 'lion-hk-row';
    const typeLbl = document.createElement('span');
    typeLbl.className = 'lion-hk-row-label';
    typeLbl.textContent = 'Type';
    const typeVal = document.createElement('span');
    typeVal.className = 'lion-hk-row-value';
    typeVal.textContent = dc ? dc.charAt(0).toUpperCase() + dc.slice(1) : 'Illuminance';
    typeRow.appendChild(typeLbl); typeRow.appendChild(typeVal);
    listCard.appendChild(typeRow);
    rowCount++;

    // Unit row
    addSep();
    const unitRow = document.createElement('div');
    unitRow.className = 'lion-hk-row';
    const unitLbl = document.createElement('span');
    unitLbl.className = 'lion-hk-row-label';
    unitLbl.textContent = 'Unit';
    const unitVal = document.createElement('span');
    unitVal.className = 'lion-hk-row-value';
    unitVal.textContent = unit;
    unitRow.appendChild(unitLbl); unitRow.appendChild(unitVal);
    listCard.appendChild(unitRow);

    // Threshold row
    addSep();
    const thrRow = document.createElement('div');
    thrRow.className = 'lion-hk-row';
    const thrLbl = document.createElement('span');
    thrLbl.className = 'lion-hk-row-label';
    thrLbl.textContent = 'Bright threshold';
    const thrVal = document.createElement('span');
    thrVal.className = 'lion-hk-row-value';
    thrVal.textContent = `${cfg.high_threshold ?? 1000} lx`;
    thrRow.appendChild(thrLbl); thrRow.appendChild(thrVal);
    listCard.appendChild(thrRow);

    // Last changed row
    addSep();
    const lastRow = document.createElement('div');
    lastRow.className = 'lion-hk-row tappable';
    const lastLbl = document.createElement('span');
    lastLbl.className = 'lion-hk-row-label';
    lastLbl.textContent = 'Last updated';
    const lastRight = document.createElement('div');
    lastRight.className = 'lion-hk-row-value';
    const lastValSpan = document.createElement('span');
    lastValSpan.textContent = this._timeAgo(getLastChanged());
    const chev = document.createElement('span');
    chev.className = 'lion-hk-chevron'; chev.textContent = '›';
    ...
    this._refreshSensorPopup = () => {
      const isHigh = getIsHigh();
      const luxVal = getLux();
      applyStateCircle(isHigh, luxVal);
      applyLuxLabel(isHigh, luxVal);
      applyStateLabel(isHigh);
      lastValSpan.textContent = this._timeAgo(getLastChanged());
    };
...
//  Visual Editor
// ─────────────────────────────────────────────────────────────────────────────
class LionLuxCardEditor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' });
    this._config      = {};
  ...
  _syncFieldValues(prev) {
    ...
    maybeSet(root.getElementById('high-threshold'), cfg.high_threshold  ?? 1000);
    maybeSet(root.getElementById('fill-max'),       cfg.fill_max        ?? 10000);
    ...
    LION_COLOUR_FIELDS.forEach(field => {
  ...
  _syncEntityChecks() {
    ...
    root.querySelectorAll('.check-item').forEach(item => {
      ...
      const fnR = item.querySelector('.fn-row');
      if (!cb) return;
      const isChecked = selected.includes(id);
      cb.checked     = isChecked;
      item.draggable = isChecked;
      if (fnR) fnR.classList.toggle('visible', isChecked);
      const fnIn = item.querySelector('.fn-input');
      if (fnIn) fnIn.value = fn[id] || '';
    });
  }
  ...
  _renderEditor() {
    ...
    // Gather all illuminance / lux sensors, sorted: illuminance first, then all numeric sensors
    const RELEVANT_DC = new Set(['illuminance']);
    const states = this._hass?.states || {};

    const allSensorIds = Object.keys(states)
      ...
      .filter(id => {
        ...
        // Include illuminance device class, or sensors reporting lux
        return RELEVANT_DC.has(dc) || unit === 'lx' || unit === 'lux';
      });

    // Sort: relevant first
    allSensorIds.sort((a, b) => {
      const dcA = states[a]?.attributes?.device_class || '';
      const dcB = states[b]?.attributes?.device_class || '';
      const relA = RELEVANT_DC.has(dcA) ? 0 : 1;
      const relB = RELEVANT_DC.has(dcB) ? 0 : 1;
      if (relA !== relB) return relA - relB;
      return (states[a]?.attributes?.friendly_name || a).localeCompare(states[b]?.attributes?.friendly_name || b);
    });

    this._allEntities = allSensorIds;

    this.shadowRoot.innerHTML = `
      <style>
        :host { display: block; font-family: var(--primary-font-family, inherit); }
        .section { margin-bottom: 16px; }
        .section-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--secondary-text-color); padding: 0 2px 8px; display: flex; align-items: center; gap: 6px; }
        .card-block { background: var(--ha-card-background, var(--card-background-color)); border-radius: 12px; overflow: hidden; border: 1px solid var(--divider-color, rgba(0,0,0,0.1)); }
        .field-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 14px; border-bottom: 1px solid var(--divider-color, rgba(0,0,0,0.07)); }
        ...
        .text-input { padding: 6px 10px; border: 1px solid var(--divider-color, rgba(0,0,0,0.15)); border-radius: 8px; background: var(--secondary-background-color); color: var(--primary-text-color); font-size: 13px; font-family: inherit; outline: none; -webkit-appearance: none; width: 130px; flex-shrink: 0; }
        .text-input:focus { border-color: #FFD60A; }
        .num-input  { padding: 6px 10px; border: 1px solid var(--divider-color, rgba(0,0,0,0.15)); border-radius: 8px; background: var(--secondary-background-color); color: var(--primary-text-color); font-size: 13px; font-family: inherit; outline: none; -webkit-appearance: none; width: 90px; flex-shrink: 0; text-align: right; }
        .num-input:focus { border-color: #FFD60A; }
        .search-wrap { padding: 10px 12px; border-bottom: 1px solid var(--divider-color, rgba(0,0,0,0.07)); }
        .search-box { width: 100%; box-sizing: border-box; padding: 8px 10px; border: 1px solid var(--divider-color, rgba(0,0,0,0.12)); border-radius: 8px; background: var(--secondary-background-color); color: var(--primary-text-color); font-size: 13px; font-family: inherit; outline: none; -webkit-appearance: none; }
        .search-box:focus { border-color: #FFD60A; }
        .checklist { max-height: 280px; overflow-y: auto; }
        .check-item { display: flex; flex-direction: column; padding: 6px 12px; border-bottom: 1px solid var(--divider-color, rgba(0,0,0,0.06)); background: transparent; transition: background 0.12s; }
        ...
        .dc-badge { font-size: 9px; background: rgba(255,214,10,0.15); color: #FFD60A; border: 1px solid rgba(255,214,10,0.3); border-radius: 5px; padding: 1px 5px; font-weight: 700; text-transform: capitalize; vertical-align: middle; margin-left: 4px; }
        ...
        .auto-badge { font-size: 9px; background: #FFD60A22; color: #FFD60A; border: 1px solid #FFD60A44; border-radius: 6px; padding: 1px 6px; font-weight: 700; }
      </style>
      ...
      <div class="section">
        ...
        <div class="card-block">
          <div class="field-row">
            ...
            <input class="text-input" id="title" type="text" value="${cfg.title || ''}" placeholder="e.g. Light Levels" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">
          </div>
          <div class="field-row">
            <div>
              <div class="field-label">Bright threshold <span style="font-size:10px;color:var(--secondary-text-color);font-weight:400;">(lx)</span></div>
              <div class="field-desc">Sensors at or above this lux value are marked Bright.</div>
            </div>
            <input class="num-input" id="high-threshold" type="number" min="0" step="50" value="${cfg.high_threshold ?? 1000}">
          </div>
          <div class="field-row">
            <div>
              <div class="field-label">Fill bar max <span style="font-size:10px;color:var(--secondary-text-color);font-weight:400;">(lx)</span></div>
              <div class="field-desc">Lux value at which the pill fill bar reaches 100%.</div>
            </div>
            <input class="num-input" id="fill-max" type="number" min="100" step="500" value="${cfg.fill_max ?? 10000}">
          </div>
          <div class="field-row">
            <div>
      ...
      <div class="section">
        ...
        <div style="font-size:10px;color:var(--secondary-text-color);padding:4px 2px;">
          Shows illuminance sensors (device class: illuminance, or unit: lx). Toggle to select, drag grip to reorder.
        </div>
      </div>
    ...
    const highThreshEl = this.shadowRoot.getElementById('high-threshold');
    highThreshEl.addEventListener('blur', () => { const v = parseInt(highThreshEl.value); if (!isNaN(v) && v >= 0) this._commitConfig('high_threshold', v); });
    highThreshEl.addEventListener('keydown', e => { if (e.key === 'Enter') highThreshEl.blur(); });

    const fillMaxEl = this.shadowRoot.getElementById('fill-max');
    fillMaxEl.addEventListener('blur', () => { const v = parseInt(fillMaxEl.value); if (!isNaN(v) && v >= 100) this._commitConfig('fill_max', v); });
    fillMaxEl.addEventListener('keydown', e => { if (e.key === 'Enter') fillMaxEl.blur(); });
  ...
  _renderEntityList() {
    ...
    if (!all.length) {
      list.innerHTML = `<div style="padding:14px;font-size:12px;color:var(--secondary-text-color);">No illuminance / lux sensor entities found in Home Assistant.</div>`;
      return;
    }
    ...
    ordered.forEach(entityId => {
      ...
      const luxVal    = parseFloat(stateObj?.state);
      const luxStr    = !isNaN(luxVal) ? `${Math.round(luxVal)} lx` : stateObj?.state || '—';
      const dc        = stateObj?.attributes?.device_class || '';
      const isRel     = dc === 'illuminance';
      ...
      item.innerHTML = `
        <div class="check-item-row">
          ...
          <span class="entity-state" style="color:${!isNaN(luxVal) && luxVal >= (this._config.high_threshold ?? 1000) ? '#FF9F0A' : 'rgba(120,120,128,0.6)'};">${luxStr}</span>
          <label class="toggle-switch">
  ...
  _commitConfig(key, value) {
    this._config = { ...this._config, [key]: value, type: 'custom:lion-lux-card' };
...
// ─── Registration ─────────────────────────────────────────────────────────────
if (!customElements.get('lion-lux-card')) {
  customElements.define('lion-lux-card', LionLuxCard);
}
if (!customElements.get('lion-lux-card-editor')) {
  customElements.define('lion-lux-card-editor', LionLuxCardEditor);
}

window.customCards = window.customCards || [];
if (!window.customCards.some(c => c.type === 'lion-lux-card')) {
  window.customCards.push({
    type:        'lion-lux-card',
    name:        'Lion Lux Card',
    preview:     true,
    description: 'Compact pill card showing current lux levels at a glance, with a full overview popup and 24-hour history for each sensor.',
  });
}
